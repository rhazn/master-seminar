...
\note{
    \begin{itemize}
        ...
        \item Das papier bespricht vieles, wir konzentrieren uns auf: operatoren die die weltanschauung eines agenten unter neuen informationen anpassen, nur tpo revision an sich
...
\begin{frame}{Table of Contents}
    ...
    \note{
        \begin{itemize}
            \item Zentral informal: Funktion * die für einen Status und neue Informationen einen neuen Status ermitteln
            ...
            \item Benötigte neue Metadaten, Operator für tpo revision definieren, besprechen wie man diese neuen strukturen anpassen kann
            \item Dann: Wie bauen Booth und Meyer auf der vorherigen Arbeit auf um tpos zu ändern, welche neuen metadaten/strukturen brauche sie dafür?
            \item BM operatoren: Wie benutzen BM die vorher besprochenen Metadaten, Definition von Eigenschaften ihrer Operatoren
            \item Iteration: Ausblick auf mehr als einen Änderungsschritt + konkreter Operator für die änderung der neuen struktur
        \end{itemize}
    }
\end{frame}
...
\begin{frame}{Research Context}
    ...
    \note{
        \begin{itemize}
            \item Einordnung des Papiers in ein Forschungsgebiet
...
\begin{frame}{Different types of belief}
    \begin{itemize}
        ...
        \item Strategy to change conditional beliefs \cite{Booth2011}
    \end{itemize}
...
\begin{frame}{Research question}
    ...
    \note{
        \begin{itemize}
            ...
            \item "conditional belief revision  operators" = tpo revision operatoren
...
\begin{frame}{Courtroom example \footnote{inspired by \cite{Booth2011} and \cite{Darwiche1997}}}
    ...
    \begin{itemize}
        ...
        \item Lower case greek letters used for formulas $\alpha$
    \end{itemize}
...
\begin{frame}{Belief Set Revision Postulates\footnote{AGM postulates \cite{Alchourron1985}}}
    \label{slide:e1-8}
    \begin{itemize}
        \item AGM theory for belief set revision
        \item minimal change for belief set with new information
        \item no restrictions on the changes in conditional beliefs
    \end{itemize}
    ...
    \note{
        \begin{itemize}
            ...
            \item 3 und 4: zusammen, wenn neues belief ist konsistent mit dem vorherigen belief set dann gibt es keinen grund beliefs zu entfernen. das neue belief set wird vorheriges belief set + alpha + was aus alpha und belief set folgt sein, minimal change wenn alpha konsistent mit belief set
            ...
            \item 7 und 8: Wenn ein belief set nach revision mit alpha konsistent ist mit beta dann kann man das belief set von alpha und beta durch expansion mit beta erreichen (revision nicht nötig), wieder minimal change
        \end{itemize}
    }
\end{frame}
...
\begin{frame}{Epistemic States}
    ...
    \note{
        \begin{itemize}
            ...
            \item Belief sets können von epistemischen Staten extrahiert werden als formeln die in allen plausibelsten Welten wahr sind
            \item Was sind plausibelste welten? tpo!
...
\begin{frame}{Enriching Epistemic States}
    ...
    \note{
        \begin{itemize}
            ...
            \item idee: welten können sogar wenn sie von informationen unterstützt werden als nicht so plausibel wie andere welten angesehen werden. e.g. welten in denen es aliens gibt glaubt man eventuell nicht, selbst wenn man informationen erhält die für sie sprechen
        \end{itemize}
    }
\end{frame}
...
\note{
    \begin{itemize}
        ...
        \item Uns interessieren funktionen * die für eine tpo und alle alpha aus L eine neue tpo generieren
...
\begin{frame}{Properties of BM Revision Operators: Common rules in iterated belief change}
    ...
    \note{
        ...
        \begin{itemize}
            ...
            \item 5: eine welt x ist mindestens so plausibel ist wie eine Welt y. x soll nach einer Eingabe, für die x ein model ist und y nicht model ist, strikt plausibler sein (ähnlich wie CR3/4 von DP aber strenger)
...
\begin{frame}{Family of BM Revision Operators}
    ...
    \note{
        \begin{itemize}
            \item theorem charakterisiert die familie von operatoren die booth und meyer diskutieren
            \item funktionen * die aus einer tpo und einer formel in L eine neue tpo erzeugen
            \item erfüllt immer die postulate wenn sie aus den vorgegebenen strukturen generiert wird
            \item für jeden revisionsoperator * für eine tpo kann man eine faithful tpo generieren die den gleichen operator erzeugt nach der definition hier
        \end{itemize}
    }
\end{frame}
...
\begin{frame}[fragile]{Non-priotized revision}
    ...
    \note{
        \footnotesize
        \begin{itemize}
            \item (OPTIONAL oder sehr schnell)
...
\note{
    \begin{itemize}
        ...
        \item Jetzt: Wie die Revision von Annahmen funktioniert für mehr als einen Input, iterated tpo revision!
        \item TAKES: 10-15
    \end{itemize}
}
...
\begin{frame}[fragile]{Courtroom Example: Visualised}
    ...
    \note{
        \begin{itemize}
            ...
            \item was hat sich jetzt geändert? die konditionalen annahmen, tpo revision was der agent bereit ist zu akzeptieren mit neuen beweisen
            \item aber auch: die strategie für zukünftige tpo revision (W+- und faithful tpo)
        \end{itemize}
    }
\end{frame}
...
\begin{frame}[fragile]{Courtroom Example: Conditional beliefs 1}
    ...
    \note{
        \begin{itemize}
            ...
            \item trotz der evidenz ist "das opfer ist ein alien" nicht teil der minimalen welten und damit nicht teil des belief sets (non priotized revision)
            \item aber die strategie für konditionalen annahmen revision hat sich jetzt verändert, der abstand zwischen den alien welten und nicht alien welten ist weg
